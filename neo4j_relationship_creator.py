...
authenticate("localhost:7474", "neo4j", "hello")
graph = Graph("http://localhost:7474/db/data/")
print("yolo")

# Clear the database first
query = """ MATCH (n:Video) DELETE n """
graph.run(query)
query = """ MATCH (n:Youtube) DELETE n """
graph.run(query)

arrayjson = []
filelist = os.listdir("/docs_and_data/test")
for i in range(len(filelist)):
    filelist[i] = "/home/hiten/Downloads/vse/test/" + filelist[i]
...
for i in range(len(arrayjson)):
    ...
    a = Node("Youtube", name=arrayjson[i]['videoInfo']['id'], commentCount=arraystring['commentCount'],
             viewCount=arraystring['viewCount'], favoriteCount=arraystring['favoriteCount'],
             dislikeCount=arraystring['dislikeCount'], likeCount=int(arraystring['likeCount']))
    graph.create(a)
    print("yolo " + str(i))
...
for i in range(len(arrayjson)):
    ...
    for j in range(i - 1, -1, -1):
        ...
        Count = descriptionCompare(arrayjson[i]['videoInfo']['snippet']['description'],
        ...
        if 'tags' in arrayjson[i]['videoInfo']['snippet'] and 'tags' in arrayjson[j]['videoInfo']['snippet']:
            tagCount = tagsCompare(arrayjson[i]['videoInfo']['snippet']['tags'],
                                   arrayjson[j]['videoInfo']['snippet']['tags'])
            if tagCount != 0:
                a = graph.find_one("Youtube", property_key='name', property_value=element['videoInfo']['id'])
                b = graph.find_one("Youtube", property_key='name', property_value=arrayjson[j]['videoInfo']['id'])
